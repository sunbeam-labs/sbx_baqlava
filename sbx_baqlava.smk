try:
    SBX_TEMPLATE_VERSION = get_ext_version("sbx_baqlava")
except NameError:
    # For backwards compatibility with older versions of Sunbeam
    SBX_TEMPLATE_VERSION = "0.0.0"

try:
    logger = get_extension_logger("sbx_baqlava")
except NameError:
    # For backwards compatibility with older versions of Sunbeam
    import logging

    logger = logging.getLogger("sunbeam.pipeline.extensions.sbx_baqlava")


logger.info("Doing some extension specific setup...")
logger.info(f"Using sbx_baqlava version {SBX_TEMPLATE_VERSION}.")
...
rule example_rule:
    ...
    params:
        opts=Cfg["sbx_baqlava"]["example_rule_options"],
    conda:
        "envs/sbx_baqlava_env.yml"
    container:
        f"docker://sunbeamlabs/sbx_baqlava:{SBX_TEMPLATE_VERSION}"
    shell:
